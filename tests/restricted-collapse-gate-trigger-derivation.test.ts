...
const artifactPath = path.join(
  process.cwd(),
  "artifacts/gravity/restricted-collapse-gate-trigger-derivation_2026_05_22.json",
);

const artifact = JSON.parse(fs.readFileSync(artifactPath, "utf8"));

describe("restricted-collapse-gate-trigger-derivation", () => {
  it("records the supplied restricted-chain object", () => {
    expect(artifact.id).toBe("RESTRICTED_COLLAPSE_GATE_TRIGGER_DERIVATION");
    expect(artifact.supplied_object).toBe("RESTRICTED_COLLAPSE_GATE_TRIGGER_DERIVATION");
    expect(artifact.status).toBe("DERIVATION_RECORDED_RESTRICTED_ONLY");
  });

  it("records the object kind", () => {
    expect(artifact.object_kind).toBe("restricted_derivation_object");
  });

  it("records the weakest sufficient next object", () => {
    expect(artifact.weakest_sufficient_next_object).toBe("RESTRICTED_COLLAPSE_GATE_THEOREM_PROOF_OBJECT");
  });
