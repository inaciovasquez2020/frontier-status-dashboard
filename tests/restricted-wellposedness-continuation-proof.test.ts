...
const artifactPath = path.join(
  process.cwd(),
  "artifacts/gravity/restricted-wellposedness-continuation-proof_2026_05_22.json",
);

const artifact = JSON.parse(fs.readFileSync(artifactPath, "utf8"));

describe("restricted-wellposedness-continuation-proof", () => {
  it("records the supplied restricted-chain object", () => {
    expect(artifact.id).toBe("RESTRICTED_WELLPOSEDNESS_CONTINUATION_PROOF");
    expect(artifact.supplied_object).toBe("RESTRICTED_WELLPOSEDNESS_CONTINUATION_PROOF");
  ...
  it("records the weakest sufficient next object", () => {
    expect(artifact.weakest_sufficient_next_object).toBe("RESTRICTED_COLLAPSE_GATE_TRIGGER_DERIVATION");
  });
